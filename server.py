...
def fetch_stats_by_id(player_id, player_name_slug):
    ...
    slugs_to_try = list(dict.fromkeys([
        player_name_slug,
        player_name_slug.lower().replace(" ", "-"),
        re.sub(r'[^a-z0-9-]', '', player_name_slug.lower().replace(" ", "-")),
        "player",
    ]))
    last_error = None
    for slug in slugs_to_try:
        url = f"https://cricheroes.com/player-profile/{player_id}/{slug}?tab=matches"
        response = requests.get(url, cookies=cookies, headers=headers)
        text = response.text
        start = text.find('"playerInfo":')
        if start == -1:
            continue
        chunk = text[start:start+100]
        if '"code":401' in chunk:
            last_error = "private"
            continue
        if '"status":false' in chunk:
            continue
    ...
    if last_error == "private":
        return {"error": "private", "player_id": player_id, "slug": player_name_slug}
    return None
...
class Handler(SimpleHTTPRequestHandler):
    def do_GET(self):
        ...
        if parsed.path == '/search-player':
            ...
            if stats and stats.get("error") == "private":
